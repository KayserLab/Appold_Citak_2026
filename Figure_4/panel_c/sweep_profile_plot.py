...
fig, ax1 = plt.subplots(figsize=(3.3774, 1.7147), dpi=300)  # (7.25/3.5, 7.1/6)

ax1.set_ylabel('TTP (h)', color='c')
ax1.set_xlabel(r'$\tau_{\mathrm{on}}$ (h)', labelpad=0)
ax1.set_xticks([0, 5, 10, 15, 20])

ax1.plot(x[:41], median_total[1][:,36][:41], 'c')
print(np.argmax(median_total[1][:,36]))
ax1.fill_between(x[:41], q1_total[1][:,36][:41], q3_total[1][:,36][:41], alpha=0.5, color='c', linewidth=0)

ax2 = ax1.twinx()

ax2.set_ylabel('Resistant fraction', rotation=270, labelpad=10, color='m')
ax2.set_ylim(-0.05, 1.05)
ax2.plot(x[:41], ratio_total[1][:,36][:41], 'mx', markersize=3)
ax2.fill_between(x[:41], ratio_q1_total[1][:,36][:41], ratio_q3_total[1][:,36][:41], alpha=0.5, color='m', linewidth=0)

plt.tight_layout()
plt.savefig(fr'Figure_4/panel_c/sweep_profile_{folder}.pdf', transparent=True, bbox_inches='tight')
plt.show()
