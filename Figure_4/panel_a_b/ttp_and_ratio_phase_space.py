...
def build_sweep_indices(params):
    ...
    mutation_rates = [0.29251213431358345, 0.5850242686271669, 0.87753640294075035]  # np.linspace(params['mutation_rate_min'], params['mutation_rate_max'], num_mutation_rate_steps)
...
def plot_existing_data(folder):
    ...
    test = 'rebuttal_sweep_final'
    params = torch.load(f'data/sweeps/{test}/params.pth', map_location='cpu', weights_only=False)
    ...
    for mutation_rate in range(mutation_rates_len):
        ...
        fig, axs = plt.subplots(figsize=(3.6, 2.4))
        ...
        axs.plot(range(len(max_line))[23:81],max_line[23:81], color='black', linestyle=':', linewidth=1, zorder=0, label=r'$\tau*$')
        # im0 = axs.imshow(mean_total[mutation_rate], interpolation='none', cmap=cmap, origin='lower', vmin=mean_total[mutation_rate].min(), vmax=mean_total[mutation_rate].max())
        # axs.plot(range(len(max_line)), max_line, color='black', linestyle=':', linewidth=1, zorder=0, label=r'$\tau*$')
        # axs.set_title(f'{mutation_rates[mutation_rate]}', fontsize=7)
        axs.set_xlabel(r'$\tau_{\mathrm{off}}$ (h)', labelpad=1)
        ...
        axs.set_ylabel(r'$\tau_{\mathrm{on}}$ (h)', labelpad=1)
        axs.text(3, 34, 'overtreatment', color='white', fontdict=None, zorder=5)
        axs.text(54, 3, 'undertreatment', color='white', fontdict=None)
        axs.vlines(36, 0, 40, color='c', linestyle='--', zorder=0, linewidth=1)

        cbar0 = plt.colorbar(im0, ax=axs, pad=0.04, shrink=0.56)
        ...
        plt.savefig(fr'Figure_4/panel_a_b/sweep_plots/{mutation_rates[mutation_rate]}_{folder}_median_test.pdf', transparent=True, bbox_inches='tight')
        plt.show()
        plt.close()
        ...
        fig, axs = plt.subplots(figsize=(3.6, 2.4)) # (7.5/3, 2.6*0.65)
        im1 = axs.imshow(mean_resistant[mutation_rate][:41, :81], cmap=cmap, origin='lower', interpolation='none', vmin=0, vmax=1)
        axs.plot(range(len(max_line))[23:81],max_line[23:81], color='white', linestyle=':', linewidth=1, zorder=0)
        ...
        # axs.plot(range(len(max_line)), max_line, color='white', linestyle=':', linewidth=1, zorder=0)
        axs.set_xticks(np.arange(0, treat_off_len, 2*steps_x))
        axs.set_xticklabels(x_ticks[::2*steps_x])
        axs.set_yticks(np.arange(0, treat_on_len, 2*steps_y))
        axs.set_yticklabels(y_ticks[::2*steps_y])
        ...
        axs.text(54, 3, 'undertreatment', color='black', fontdict=None)
        axs.vlines(36, 0, 40, color='m', linestyle='--', zorder=0, linewidth=1)

        cbar1 = plt.colorbar(im1, ax=axs, pad=0.04, shrink=0.56)
        ...
        plt.tight_layout()
        plt.savefig(fr'Figure_4/panel_a_b/sweep_plots/{mutation_rates[mutation_rate]}_{folder}_ratio_test.pdf', transparent=True, bbox_inches='tight')
        plt.show()
        plt.close()
