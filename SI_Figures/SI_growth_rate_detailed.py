...
- Reads 2nd sheet of each XLSX
- Adds Time column (10 min steps)
- Filters exponential window (OD between min_y and max_y)
- Fits log(OD) = m*t + b -> growth speed = m
- Computes doubling time and divisions per hour (60/doubling_time)

Outputs:
1) Growth speed vs temperature (mean±SD across wells)
2) Divisions per hour vs temperature (mean±SD across wells)
...
files_by_temp = {
    28: [
        r"S:\Members\Nico\Platereader\yNA16_s_excel_files\20230704_yNA16_S_28C.xlsx"
    ],
    29: [
        r"S:\Members\Nico\Platereader\yNA16_s_excel_files\20230712_yNA16_S_29C.xlsx"
    ],
    30: [   # two replicates
        r"S:\Members\Nico\Platereader\yNA16_s_excel_files\20230706_yNA16_S_30C.xlsx",
        r"S:\Members\Nico\Platereader\yNA16_s_excel_files\20230808_yNA16_S_30C.xlsx",
    ],
    31: [
        r"S:\Members\Nico\Platereader\yNA16_s_excel_files\20230706_yNA16_S_31C.xlsx"
    ],
    32: [
        r"S:\Members\Nico\Platereader\yNA16_s_excel_files\20230706_yNA16_S_32C.xlsx"
    ],
    35: [
        r"S:\Members\Nico\Platereader\yNA16_s_excel_files\20230705_yNA16_S_35C.xlsx"
    ],
}

# Same well groups as your script
well_groups = {
    'yNA16':  ['A1', 'B1', 'C1', 'A2', 'B2', 'C2', 'A3', 'B3', 'C3'],
    'yNA16S': ['A6', 'B6', 'C6', 'A7', 'B7', 'C7', 'A8', 'B8', 'C8'],
}

# Exponential phase window (same idea as yours)
MIN_Y = 0.02
MAX_Y = 0.6
MIN_POINTS = 10  # same threshold you use

# Style (same spirit as your Nature-style function)
mpl.rcParams.update({
    ...
    for strain, wells in well_groups.items():
        ...
        for well in wells:
            if well not in df.columns:
                # if a well is missing in this file, skip gracefully
                continue
            ...
            if len(filtered) >= MIN_POINTS:
                ...
                m, b = linear_fit(time_filtered, log_data)
                gs = m
...
def mean_and_sd(values: list[float]) -> tuple[float, float]:
    ...
    # handle case where all are inf/empty
...
def plot_metric_vs_temperature(results_by_temp: dict, metric_key: str, ylabel: str, title: str, save_path: str | None = None):
    """
    ...
    for strain in ['yNA16', 'yNA16S']:
        ...
        for t in temps:
            vals = results_by_temp[t][strain]["div_per_hr"]
    ...
    # Ensure all temps are shown as ticks
    ax.set_xticks(temps)
...
def main():
    ...
    for temp, paths in files_by_temp.items():

        # pool all files of same temperature
        pooled = {
    ...
    plot_metric_vs_temperature(
        ...
        save_path=r"S:\Members\Nico\Manuscript_Figures\For Supplement\Growth_speed_plots\growth_speed_vs_temp.pdf"
    )
    ...
    plot_metric_vs_temperature(
        ...
        save_path=r"S:\Members\Nico\Manuscript_Figures\For Supplement\Growth_speed_plots\divisions_per_hour_vs_temp.pdf"
    )
